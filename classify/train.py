...
Usage - Single-GPU training:
    $ python classify/train.py --model nexuss-cls.pt --data imagenette160 --epochs 5 --img 224

Usage - Multi-GPU DDP training:
    $ python -m torch.distributed.run --nproc_per_node 4 --master_port 2022 classify/train.py --model nexuss-cls.pt --data imagenet --epochs 5 --img 224 --device 0,1,2,3

Datasets:           --data mnist, fashion-mnist, cifar10, cifar100, imagenette, imagewoof, imagenet, or 'path/to/data'
YOLOv5-cls models:  --model nexusn-cls.pt, nexuss-cls.pt, nexusm-cls.pt, nexusl-cls.pt, nexusx-cls.pt
...
def train(opt, device):
    ...
    with torch_distributed_zero_first(LOCAL_RANK), WorkingDirectory(ROOT):
        if Path(opt.model).is_file() or opt.model.endswith(".pt"):
            ...
            m = hub.list("ultralytics/nexus")  # + hub.list('pytorch/vision')  # models
            raise ModuleNotFoundError(f"--model {opt.model} not found. Available models are: \n" + "\n".join(m))
        if isinstance(model, DetectionModel):
            LOGGER.warning("WARNING ⚠️ pass YOLOv5 classifier model with '-cls' suffix, i.e. '--model nexuss-cls.pt'")
    ...
    if RANK in {-1, 0} and final_epoch:
        LOGGER.info(
            ...
            f"\nPyTorch Hub:     model = torch.hub.load('ultralytics/nexus', 'custom', '{best}')"
...
def parse_opt(known=False):
    ...
    parser.add_argument("--model", type=str, default="nexuss-cls.pt", help="initial weights path")
...
def run(**kwargs):
    ...
    Example: from nexus import classify; classify.train.run(data=mnist, imgsz=320, model='nexusm')
    """
    opt = parse_opt(True)
