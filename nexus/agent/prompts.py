...
from nexus.dom.views import NodeType, SimplifiedNode
from nexus.llm.messages import ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage
from nexus.observability import observe_debug
from nexus.utils import is_new_tab_page, sanitize_surrogates
...
class SystemPrompt:
	def __init__(
		...
		self.is_nexus_model = is_nexus_model
	...
	def _load_prompt_template(self) -> None:
		...
		try:
			...
			if self.is_nexus_model:
				if self.flash_mode:
					template_filename = 'system_prompt_nexus_flash.md'
				elif self.use_thinking:
					template_filename = 'system_prompt_nexus.md'
				else:
					template_filename = 'system_prompt_nexus_no_thinking.md'
			...
			with (
				importlib.resources.files('nexus.agent.system_prompts')
