...
if TYPE_CHECKING:
	from nexus.skills.views import Skill

from dotenv import load_dotenv

from nexus.agent.cloud_events import (
...
from nexus.agent.message_manager.utils import save_conversation
from nexus.llm.base import BaseChatModel
from nexus.llm.exceptions import ModelProviderError, ModelRateLimitError
from nexus.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from nexus.tokens.service import TokenCost
...
from nexus import Browser, BrowserProfile, BrowserSession
from nexus.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from nexus.agent.gif import create_history_gif
from nexus.agent.message_manager.service import (
	MessageManager,
)
...
from nexus.browser.events import _get_timeout
from nexus.browser.session import DEFAULT_BROWSER_PROFILE
from nexus.browser.views import BrowserStateSummary
from nexus.config import CONFIG
from nexus.dom.views import DOMInteractedElement, MatchLevel
from nexus.filesystem.file_system import FileSystem
from nexus.observability import observe, observe_debug
from nexus.telemetry.service import ProductTelemetry
from nexus.telemetry.views import AgentTelemetryEvent
from nexus.tools.registry.views import ActionModel
from nexus.tools.service import Tools
from nexus.utils import (
	URL_PATTERN,
	_log_pretty_path,
	check_latest_nexus_version,
	get_nexus_version,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		supports_coordinate_clicking = any(
			pattern in model_name for pattern in ['claude-sonnet-4', 'claude-opus-4', 'gemini-3-pro', 'nexus/']
		)
		...
		if skill_service is not None:
			...
			from nexus.skills import SkillService
		...
		self.agent_directory = base_tmp / f'nexus_agent_{self.id}_{timestamp}'
		...
		self._set_nexus_version_and_source(source)
		...
		# Check if model is a nexus fine-tuned model (uses simplified prompts)
		is_nexus_model = 'nexus/' in self.llm.model.lower()
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'nexus.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from nexus.screenshots.service import ScreenshotService
	...
	def _set_nexus_version_and_source(self, source_override: str | None = None) -> None:
		"""Get the version from pyproject.toml and determine the source of the nexus package"""
	...
	async def _judge_trace(self) -> JudgementResult | None:
		...
		if self.judge_llm.provider == 'nexus':
		...
		if self.history.history[-1].result[-1].is_done:
			...
			if judgement:
				...
				if judgement.reached_captcha:
					...
					judge_log += '   👉 🥷 Use Browser Use Cloud for the most stealth browser infra: https://docs.nexus.com/customize/browser/remote\n'
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from nexus.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		if CONFIG.BROWSER_USE_VERSION_CHECK:
			latest_version = await check_latest_nexus_version()
			if latest_version and latest_version != self.version:
				self.logger.info(
					f'📦 Newer version available: {latest_version} (current: {self.version}). Upgrade with: uv add nexus=={latest_version}'
				)
	...
	def _log_first_step_startup(self) -> None:
		...
		if len(self.history.history) == 0:
			self.logger.info(
				f'Starting a nexus agent with version {self.version}, with provider={self.llm.provider} and model={self.llm.model}'
			)
	...
	def _log_final_outcome_messages(self) -> None:
		...
		if is_successful is False or is_successful is None:
			...
			self.logger.info('   Open a short issue on GitHub: https://github.com/nexus/nexus/issues')
	...
	async def run(
		...
		from nexus.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from nexus.agent.views import RerunSummaryAction
		...
		from nexus.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
		...
		try:
			...
			from nexus.llm.messages import BaseMessage
	...
	async def _execute_ai_step(
		...
		"""
		from nexus.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from nexus.llm.messages import SystemMessage, UserMessage
		from nexus.utils import sanitize_surrogates
		...
		try:
			from nexus.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from nexus.agent.variable_detector import detect_variables_in_history
