"""Cloud browser service integration for nexus.

This module provides integration with the nexus cloud browser service.
...
from nexus.browser.cloud.views import CloudBrowserAuthError, CloudBrowserError, CloudBrowserResponse, CreateBrowserRequest
from nexus.sync.auth import CloudAuthConfig

logger = logging.getLogger(__name__)


class CloudBrowserClient:
	"""Client for nexus cloud browser service."""

	def __init__(self, api_base_url: str = 'https://api.nexus.com'):
	...
	async def create_browser(
		...
		"""Create a new cloud browser instance. For full docs refer to https://docs.cloud.nexus.com/api-reference/v-2-api-current/browsers/create-browser-session-browsers-post
		...
		if not api_token:
			raise CloudBrowserAuthError(
				'No authentication token found. Please set BROWSER_USE_API_KEY environment variable to authenticate with the cloud service. You can also create an API key at https://cloud.nexus.com/new-api-key'
			)
		...
		try:
			...
			if response.status_code == 401:
				raise CloudBrowserAuthError(
					'Authentication failed. Please make sure you have set BROWSER_USE_API_KEY environment variable to authenticate with the cloud service. You can also create an API key at https://cloud.nexus.com/new-api-key'
				)
			elif response.status_code == 403:
				raise CloudBrowserAuthError('Access forbidden. Please check your nexus cloud subscription status.')
