...
_DEMO_PANEL_SCRIPT = r"""(function () {
  ...
  const EXCLUDE_ATTR = 'data-nexus-exclude-' + SESSION_ID;
  const PANEL_ID = 'nexus-demo-panel';
  const STYLE_ID = 'nexus-demo-panel-style';
  ...
  const TOGGLE_BUTTON_ID = 'nexus-demo-toggle';
  ...
  function initializePanel() {
    ...
    if (savedWidth) {
      document.documentElement.style.setProperty('--nexus-demo-panel-width', `${savedWidth}px`);
    }
  ...
  function addStyles() {
    ...
    style.textContent = `
      #${PANEL_ID} {
        ...
        top: 0;
        right: 0;
        width: var(--nexus-demo-panel-width, 340px);
      ...
      #${PANEL_ID} .nexus-logo img {
        height: 36px;
      }

      #${PANEL_ID} .nexus-header-actions {
        margin-left: auto;
        display: flex;
        align-items: center;
        gap: 8px;
      ...
      #${PANEL_ID} .nexus-close-btn:hover {
        background: rgba(255, 255, 255, 0.15);
        border-color: rgba(255, 255, 255, 0.35);
      }

      #${PANEL_ID} .nexus-demo-body {
        flex: 1;
        overflow-y: auto;
      ...
      #${PANEL_ID} .nexus-demo-body::-webkit-scrollbar {
        width: 8px;
      }

      #${PANEL_ID} .nexus-demo-body::-webkit-scrollbar-thumb {
      ...
      .nexus-demo-entry {
        ...
        animation: nexus-fade-in 0.25s ease;
        background: #000000;
      }

      .nexus-demo-entry:last-child {
      ...
      .nexus-markdown-content p {
        margin: 0 0 8px 0;
      }

      .nexus-markdown-content p:last-child {
        margin-bottom: 0;
      }

      .nexus-markdown-content h1,
      .nexus-markdown-content h2,
      .nexus-markdown-content h3 {
        margin: 8px 0 4px 0;
        font-weight: 600;
        color: #f8f9ff;
      }
      ...
      .nexus-markdown-content pre code {
      ...
      .nexus-markdown-content ul,
      .nexus-markdown-content ol {
        margin: 4px 0 4px 16px;
        padding: 0;
      }

      .nexus-markdown-content li {
        margin: 2px 0;
      }

      .nexus-markdown-content a {
        color: #60a5fa;
        text-decoration: underline;
      }

      .nexus-markdown-content a:hover {
        color: #93c5fd;
      }

      .nexus-markdown-content strong {
        font-weight: 600;
        color: #f8f9ff;
      }

      .nexus-markdown-content em {
        font-style: italic;
      }

      .nexus-demo-entry:not(.expanded) .nexus-markdown-content {
        max-height: 120px;
        overflow: hidden;
      ...
      .nexus-demo-entry.level-info { border-left-color: #60a5fa; }
      .nexus-demo-entry.level-action { border-left-color: #34d399; }
      .nexus-demo-entry.level-thought { border-left-color: #f97316; }
      .nexus-demo-entry.level-warning { border-left-color: #fbbf24; }
      .nexus-demo-entry.level-success { border-left-color: #22c55e; }
      .nexus-demo-entry.level-error { border-left-color: #f87171; }
  ...
  function buildPanel() {
    ...
    header.className = 'nexus-demo-header';
    const title = document.createElement('div');
    title.className = 'nexus-logo';
    const logo = document.createElement('img');
    logo.src = 'https://raw.githubusercontent.com/nexus/nexus/main/static/nexus-dark.png';
    logo.alt = 'Browser-use';
    logo.loading = 'lazy';
    title.appendChild(logo);
    const actions = document.createElement('div');
    actions.className = 'nexus-header-actions';
    const closeBtn = document.createElement('button');
    closeBtn.type = 'button';
    closeBtn.className = 'nexus-close-btn';
    ...
    body.className = 'nexus-demo-body';
  ...
  function adjustLayout() {
    ...
    document.documentElement.style.setProperty('--nexus-demo-panel-width', `${width}px`);
    if (state.isOpen) {
  ...
  function hydrateFromStoredMarkup() {
    ...
    try {
      ...
      if (html) {
        ...
        for (const entryNode of state.list.querySelectorAll('.nexus-demo-entry')) {
          const toggle = entryNode.querySelector('.nexus-entry-toggle');
          if (toggle) {
  ...
  function createEntryNode(entry) {
    ...
    if (messageText.length > 160) {
      ...
      toggle.type = 'button';
      toggle.className = 'nexus-entry-toggle';
  ...
  function applyPersistedExpansion(node) {
    ...
    try {
      ...
      if (id && expanded.has(id)) {
        ...
        const toggle = node.querySelector('.nexus-entry-toggle');
        if (toggle) {
  ...
  window.addEventListener('nexus-log', handleLogEvent);
})();
"""
...
(() => {{
	...
	const event = new CustomEvent('nexus-log', {{ detail }});
