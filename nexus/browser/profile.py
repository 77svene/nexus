...
from nexus.browser.cloud.views import CloudBrowserParams
from nexus.config import CONFIG
from nexus.utils import _log_pretty_path, logger
...
# Using constants from central location in nexus.config
...
class BrowserLaunchArgs(BaseModel):
	...
	def set_default_downloads_path(self) -> Self:
		...
		if self.downloads_path is None:
			...
			downloads_path = Path(tempfile.gettempdir()) / f'nexus-downloads-{unique_id}'
...
class BrowserLaunchPersistentContextArgs(BrowserLaunchArgs, BrowserContextArgs):
	...
	def validate_user_data_dir(cls, v: str | Path | None) -> str | Path:
		...
		if v is None:
			return tempfile.mkdtemp(prefix='nexus-user-data-dir-')
...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	use_cloud: bool = Field(
		default=False,
		description='Use nexus cloud browser service instead of local browser',
	)
	...
	proxy: ProxySettings | None = Field(
		default=None,
		description='Proxy settings. Use nexus.browser.profile.ProxySettings(server, bypass, username, password)',
	)
	...
	def _copy_profile(self) -> None:
		...
		if 'nexus-user-data-dir-' in user_data_str.lower():
		...
		temp_dir = tempfile.mkdtemp(prefix='nexus-user-data-dir-')
