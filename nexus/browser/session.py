...
from nexus.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError

# CDP logging is now handled by setup_logging() in logging_config.py
# It automatically sets CDP logs to the same level as nexus logs
from nexus.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
from nexus.browser.events import (
...
from nexus.browser.profile import BrowserProfile, ProxySettings
from nexus.browser.views import BrowserStateSummary, TabInfo
from nexus.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from nexus.observability import observe_debug
from nexus.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from nexus.actor.page import Page
	from nexus.browser.demo_mode import DemoMode
	from nexus.browser.watchdogs.captcha_watchdog import CaptchaWaitResult
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		...
		from nexus.skill_cli.utils import find_chrome_executable, get_chrome_profile_path, list_chrome_profiles
	...
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		...
		from nexus.skill_cli.utils import list_chrome_profiles
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'nexus.{self}')
		return logging.getLogger(f'nexus.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from nexus.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from nexus.browser.events import SaveStorageStateEvent
				...
				if self.browser_profile.use_cloud or self.browser_profile.cloud_browser_params is not None:
					...
					try:
						...
						raise CloudBrowserAuthError(
							'Authentication failed for cloud browser service. Set BROWSER_USE_API_KEY environment variable. You can also create an API key at https://cloud.nexus.com/new-api-key'
						)
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from nexus.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from nexus.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from nexus.browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog
		from nexus.browser.watchdogs.captcha_watchdog import CaptchaWatchdog

		# from nexus.browser.crash_watchdog import CrashWatchdog
		from nexus.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from nexus.browser.watchdogs.dom_watchdog import DOMWatchdog
		from nexus.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from nexus.browser.watchdogs.har_recording_watchdog import HarRecordingWatchdog
		from nexus.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from nexus.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from nexus.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from nexus.browser.watchdogs.recording_watchdog import RecordingWatchdog
		from nexus.browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from nexus.browser.watchdogs.security_watchdog import SecurityWatchdog
		from nexus.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		try:
			...
			from nexus.browser.session_manager import SessionManager
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from nexus.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from nexus.dom.views import NodeType
	...
	async def remove_highlights(self) -> None:
		...
		try:
			async with asyncio.timeout(3.0):
				...
				script = """
				(function() {
					// Remove all nexus highlight elements
					const highlights = document.querySelectorAll('[data-nexus-highlight]');
					console.log('Removing', highlights.length, 'nexus highlight elements');
					highlights.forEach(el => el.remove());

					// Also remove by ID in case selector missed anything
					const highlightContainer = document.getElementById('nexus-debug-highlights');
					...
					const orphanedTooltips = document.querySelectorAll('[data-nexus-highlight="tooltip"]');
	...
	async def highlight_interaction_element(self, node: 'EnhancedDOMTreeNode') -> None:
		...
		try:
			...
			(function() {{
				...
				container.setAttribute('data-nexus-interaction-highlight', 'true');
	...
	async def highlight_coordinate_click(self, x: int, y: int) -> None:
		...
		try:
			...
			(function() {{
				...
				// Create container
				const container = document.createElement('div');
				container.setAttribute('data-nexus-coordinate-highlight', 'true');
				container.style.cssText = `
					position: absolute;
					left: ${{x + scrollX}}px;
					top: ${{y + scrollY}}px;
	...
	async def add_highlights(self, selector_map: dict[int, 'EnhancedDOMTreeNode']) -> None:
		...
		try:
			...
			(function() {{
				...
				const existingContainer = document.getElementById('nexus-debug-highlights');
				...
				const strayHighlights = document.querySelectorAll('[data-nexus-highlight]');
				...
				const container = document.createElement('div');
				container.id = 'nexus-debug-highlights';
				container.setAttribute('data-nexus-highlight', 'container');
				...
				interactiveElements.forEach((element, index) => {{
					...
					highlight.setAttribute('data-nexus-highlight', 'element');
