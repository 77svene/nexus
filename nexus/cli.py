...
if len(sys.argv) > 1 and sys.argv[1] == 'install':
	...
	if result.returncode == 0:
		...
		print('🚀 Ready to use! Run: uvx nexus')
	else:
		print('\n❌ Installation failed')
		sys.exit(1)
	sys.exit(0)
...
if 'init' in sys.argv:
	from nexus.init_cmd import INIT_TEMPLATES
	from nexus.init_cmd import main as init_main
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'nexus_{template}.py'
	...
	try:
		...
		click.echo('\nNext steps:')
		click.echo('  1. Install nexus:')
		click.echo('     uv pip install nexus')
		click.echo('  2. Set up your API key in .env file or environment:')
		click.echo('     BROWSER_USE_API_KEY=your-key')
		click.echo('     (Get your key at https://cloud.nexus.com/new-api-key)')
...
from nexus import Agent, Controller
from nexus.agent.views import AgentSettings
from nexus.browser import BrowserProfile, BrowserSession
from nexus.logging_config import addLoggingLevel
from nexus.telemetry import CLITelemetryEvent, ProductTelemetry
from nexus.utils import get_nexus_version
...
try:
	...
	print(
		'⚠️ CLI addon is not installed. Please install it with: `pip install "nexus[cli]"` and try again.', file=sys.stderr
	)
	sys.exit(1)
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('nexus.'):
		...
		# Configure nexus logger and all its sub-loggers
		nexus_logger = logging.getLogger('nexus')
		nexus_logger.propagate = False  # Don't propagate to root logger
		nexus_logger.handlers = [log_handler]  # Replace any existing handlers
		nexus_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['nexus.Agent', 'nexus.controller', 'nexus.agent', 'nexus.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'nexus.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('nexus.on_mount')
	...
	async def action_quit(self) -> None:
		...
		self.exit()
		print('\nTry running tasks on our cloud: https://nexus.com')
		...
		with Container(id='main-container'):
			...
			with Container(id='links-panel'):
				with HorizontalGroup(classes='link-row'):
					...
					yield Link('https://nexus.com', url='https://nexus.com', classes='link-white link-url')
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/nexus/awesome-prompts',
						url='https://github.com/nexus/awesome-prompts',
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/nexus/nexus/issues',
						url='https://github.com/nexus/nexus/issues',
...
async def run_prompt_mode(prompt: str, ctx: click.Context, debug: bool = False):
	"""Run nexus in non-interactive mode with a single prompt."""
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent nexus from setting up logging at import time
	os.environ['BROWSER_USE_SETUP_LOGGING'] = 'false'
	...
	try:
		...
		try:
			from nexus.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from nexus.sync.auth import DeviceAuthClient
	...
	try:
		...
		from nexus.sync.service import CloudSync
		...
		task_event = CreateAgentTaskEvent(
			...
			llm_model='auth-flow',
			task='🔐 Complete authentication and join the nexus community',
			user_id=auth_client.temp_user_id,
			device_id=auth_client.device_id,
			done_output=None,
		...
		try:
			...
			from nexus.utils import create_task_with_error_handling
		...
		if success:
				...
				actions=[
					{
						...
						'done': {
							'success': True,
							'text': '⭐ Starred nexus/nexus repository! Welcome to the community!',
						},
					}
				],
				next_goal='⭐ Star nexus GitHub repository to join the community',
			...
			print('   Future nexus runs will now sync to the cloud.')
		else:
		...
		if task_id and sync_service:
			try:
				from nexus.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: nexus_<template>.py)')
...
def main(ctx: click.Context, debug: bool = False, **kwargs):
	...
	Examples:
	  uvx nexus --template default
	  uvx nexus --template advanced --output my_script.py
	"""
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from nexus.mcp.server import main as mcp_main
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'nexus_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: nexus_<template>.py)',
)
@click.option(
	'--force',
	'-f',
...
def init(
	...
	"""
	Generate a nexus template file to get started quickly.
	...
	uvx nexus init --template default

	\b
	# Generate advanced template with custom filename
	uvx nexus init --template advanced --output my_script.py
