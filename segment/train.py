...
Usage - Single-GPU training:
    $ python segment/train.py --data coco128-seg.yaml --weights nexuss-seg.pt --img 640  # from pretrained (recommended)
    $ python segment/train.py --data coco128-seg.yaml --weights '' --cfg nexuss-seg.yaml --img 640  # from scratch

Usage - Multi-GPU DDP training:
    $ python -m torch.distributed.run --nproc_per_node 4 --master_port 1 segment/train.py --data coco128-seg.yaml --weights nexuss-seg.pt --img 640 --device 0,1,2,3
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights", type=str, default=ROOT / "nexuss-seg.pt", help="initial weights path")
...
def run(**kwargs):
    ...
    Example: import train; train.run(data='coco128.yaml', imgsz=320, weights='nexusm.pt')
    """
    opt = parse_opt(True)
