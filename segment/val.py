...
Usage:
    ...
    $ python segment/val.py --weights nexuss-seg.pt --data coco.yaml --img 640  # validate COCO-segments

Usage - formats:
    $ python segment/val.py --weights nexuss-seg.pt                 # PyTorch
                                      nexuss-seg.torchscript        # TorchScript
                                      nexuss-seg.onnx               # ONNX Runtime or OpenCV DNN with --dnn
                                      nexuss-seg_openvino_label     # OpenVINO
                                      nexuss-seg.engine             # TensorRT
                                      nexuss-seg.mlmodel            # CoreML (macOS-only)
                                      nexuss-seg_saved_model        # TensorFlow SavedModel
                                      nexuss-seg.pb                 # TensorFlow GraphDef
                                      nexuss-seg.tflite             # TensorFlow Lite
                                      nexuss-seg_edgetpu.tflite     # TensorFlow Edge TPU
                                      nexuss-seg_paddle_model       # PaddlePaddle
"""
...
def parse_opt():
    ...
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "nexuss-seg.pt", help="model path(s)")
...
def main(opt):
    ...
    if opt.task in ("train", "val", "test"):  # run normally
        ...
        if opt.task == "speed":  # speed benchmarks
            # python val.py --task speed --data coco.yaml --batch 1 --weights nexusn.pt nexuss.pt...
            opt.conf_thres, opt.iou_thres, opt.save_json = 0.25, 0.45, False
            for opt.weights in weights:
                run(**vars(opt), plots=False)

        elif opt.task == "study":  # speed vs mAP benchmarks
            # python val.py --task study --data coco.yaml --iou 0.7 --weights nexusn.pt nexuss.pt...
            for opt.weights in weights:
