...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem https://github.com/ultralytics/nexus/issues/9287
    if deterministic and check_version(torch.__version__, "1.12.0"):  # https://github.com/ultralytics/nexus/pull/8213
...
def check_git_status(repo="ultralytics/nexus", branch="master"):
...
def check_git_info(path="."):
    ...
    try:
        repo = git.Repo(path)
        remote = repo.remotes.origin.url.replace(".git", "")  # i.e. 'https://github.com/ultralytics/nexus'
...
def check_suffix(file="nexuss.pt", suffix=(".pt",), msg=""):
...
def check_amp(model):
    ...
    try:
        assert amp_allclose(deepcopy(model), im) or amp_allclose(DetectMultiBackend("nexusn.pt", device), im)
        LOGGER.info(f"{prefix}checks passed ✅")
        return True
    except Exception:
        help_url = "https://github.com/ultralytics/nexus/issues/7908"
