...
def smart_DDP(model):
    ...
    assert not check_version(torch.__version__, "1.12.0", pinned=True), (
        ...
        "Please upgrade or downgrade torch to use DDP. See https://github.com/ultralytics/nexus/issues/8395"
    )
...
def model_info(model, verbose=False, imgsz=640):
    ...
    name = Path(model.yaml_file).stem.replace("nexus", "YOLOv5") if hasattr(model, "yaml_file") else "Model"
...
def smart_hub_load(repo="ultralytics/nexus", model="nexuss", **kwargs):
...
def smart_resume(ckpt, optimizer, ema=None, weights="nexuss.pt", epochs=300, resume=True):
